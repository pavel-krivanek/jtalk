...
fn: function () {
    ...
    expressions = expression.__comma(ws.__comma("."._asParser()).__comma(ws).__comma(expression).__eq_eq_gt(function (node) {return node._fourth();})._star()).__eq_eq_gt(function (node) {var result = nil;result = smalltalk.Array._with_(node._first());node._second()._do_(function (each) {return result._add_(each);});return result;});
...
fn: function () {
    ...
    var data = nil;
    stream = ""._writeStream();
    smalltalk.Smalltalk._current()._classes()._do_(function (each) {return stream._nextPutAll_(self._export_(each));});
    data = stream._contents();
    (function () {var dirService = Components.classes['@mozilla.org/file/directory_service;1'].getService(Components.interfaces.nsIProperties);var file = dirService.get("AChrom", Components.interfaces.nsIFile);file.append("content");file.append("snapshot.js");var foStream = Components.classes['@mozilla.org/network/file-output-stream;1'].createInstance(Components.interfaces.nsIFileOutputStream);foStream.init(file, 2 | 8 | 32, 438, 0);var converter = Components.classes['@mozilla.org/intl/converter-output-stream;1'].createInstance(Components.interfaces.nsIConverterOutputStream);converter.init(foStream, "UTF-8", 0, 0);converter.writeString(data);converter.close();}());
    return self;
},
source: unescape('saveSnapshot%0A%0A%09%7C%20stream%20data%20%7C%0A%09stream%20%3A%3D%20%27%27%20writeStream.%0A%09Smalltalk%20current%20classes%20%0A%09%20%20%20%20do%3A%20%5B%3Aeach%20%7C%20stream%20nextPutAll%3A%20%28self%20export%3A%20each%29%5D.%0A%09data%20%3A%3D%20stream%20contents.%0A%0A%20%20%7B%27%0A%20%20var%20dirService%20%3D%20Components.classes%5B%22@mozilla.org/file/directory_service%3B1%22%5D%0A%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20.getService%28Components.interfaces.nsIProperties%29%3B%20%20%20%0A%20%20var%20file%20%3D%20dirService.get%28%22AChrom%22%2C%20Components.interfaces.nsIFile%29%3B%20//%20returns%20an%20nsIFile%20object%20%20%0A%0A%20%20file.append%28%22content%22%29%3B%0A%20%20file.append%28%22snapshot.js%22%29%3B%0A%0A%20%20//%20file%20is%20nsIFile%2C%20data%20is%20a%20string%0A%20%20var%20foStream%20%3D%20Components.classes%5B%22@mozilla.org/network/file-output-stream%3B1%22%5D.%0A%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20createInstance%28Components.interfaces.nsIFileOutputStream%29%3B%0A%0A%20%20//%20use%200x02%20%7C%200x10%20to%20open%20file%20for%20appending.%0A%20%20foStream.init%28file%2C%200x02%20%7C%200x08%20%7C%200x20%2C%200666%2C%200%29%3B%20%0A%20%20//%20write%2C%20create%2C%20truncate%0A%20%20//%20In%20a%20c%20file%20operation%2C%20we%20have%20no%20need%20to%20set%20file%20mode%20with%20or%20operation%2C%0A%20%20//%20directly%20using%20%22r%22%20or%20%22w%22%20usually.%0A%0A%20%20//%20if%20you%20are%20sure%20there%20will%20never%20ever%20be%20any%20non-ascii%20text%20in%20data%20you%20can%20%0A%20%20//%20also%20call%20foStream.writeData%20directly%0A%20%20var%20converter%20%3D%20Components.classes%5B%22@mozilla.org/intl/converter-output-stream%3B1%22%5D.%0A%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20createInstance%28Components.interfaces.nsIConverterOutputStream%29%3B%0A%20%20converter.init%28foStream%2C%20%22UTF-8%22%2C%200%2C%200%29%3B%0A%20%20converter.writeString%28data%29%3B%0A%20%20converter.close%28%29%3B%20//%20this%20closes%20foStream%0A%0A%27%7D%0A')}),
smalltalk.Exporter);
...
fn: function () {
    var self = this;
    return function () {return smalltalk.Exporter._new()._saveSnapshot();}._value();
    return self;
},
source: unescape('doIt%20%5E%5BExporter%20new%20saveSnapshot%5D%20value')}),
smalltalk.DoIt);
...
fn: function (html) {
    var self = this;
    (function ($rec) {$rec._id_("jtalkTabs");return $rec._with_(function () {(function ($rec) {$rec._with_("Save snapshot");return $rec._onClick_(function () {return smalltalk.Exporter._new()._saveSnapshot();});}(html._li()));(function ($rec) {$rec._class_("closeAll");$rec._with_("x");return $rec._onClick_(function () {return self._close();});}(html._li()));self._tabs()._do_(function (each) {return self._renderTabFor_on_(each, html);});return function ($rec) {$rec._class_("newtab");$rec._with_(unescape("%20+%20"));return $rec._onClick_(function () {return self._newBrowserTab();});}(html._li());});}(html._ul()));
    return self;
},
source: unescape('renderOn%3A%20html%0A%20%20%20%20html%20ul%0A%09id%3A%20%27jtalkTabs%27%3B%0A%09with%3A%20%5B%0A%09%20%20%20%20html%20li%20%0A%09%09with%3A%20%27Save%20snapshot%27%3B%0A%09%09onClick%3A%20%5BExporter%20new%20saveSnapshot%5D.%0A%09%20%20%20%20html%20li%20%0A%09%09class%3A%20%27closeAll%27%3B%0A%09%09with%3A%20%27x%27%3B%0A%09%09onClick%3A%20%5Bself%20close%5D.%0A%09%20%20%20%20self%20tabs%20do%3A%20%5B%3Aeach%20%7C%0A%09%09self%20renderTabFor%3A%20each%20on%3A%20html%5D.%0A%09%20%20%20%20html%20li%0A%09%09class%3A%20%27newtab%27%3B%0A%09%09with%3A%20%27%20+%20%27%3B%0A%09%09onClick%3A%20%5Bself%20newBrowserTab%5D%5D%0A')}),
smalltalk.TabManager);
